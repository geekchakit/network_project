...
//     const centerX =
//       ((gateway.wtp_array?.length - 1) * ROOT_X_SPACING + 150) / 2;
...
const FlowchartVisualization = ({ data }) => {
  ...
  const ROOT_Y = 0;
  const GATEWAY_X_SPACING = 400;
  const ROOT_X_SPACING = 400;
  const CHILD_Y_SPACING = 600;
  const CHILD_X_SPACING = 260;

  const RADIO_CARD_WIDTH = 300; // Width of each radio card
  const RADIO_SPACING = 320; // Horizontal spacing between radio cards

  // Iterate through each gateway in the data array
  data.forEach((gatewayData, gatewayIndex) => {
    const gatewayId = `gateway-${gatewayIndex}`;
    const gatewayX = gatewayIndex * GATEWAY_X_SPACING;
    const centerX =
      ((gatewayData.wtp_array?.length - 1) * ROOT_X_SPACING + 150) / 2;
      ...
      data: {
        label: (
          ...
          >
            ...
            Gateway
            <span className="bg-primary/10 border border-primary/20 mb-2 text-primary font-primary font-medium py-2 rounded-full">
              {gatewayData.wtp_array[0]?.gateway || "Unknown"}
            </span>
          </div>
        ),
      },
    ...
    if (expandedGateway === gatewayId) {
      // Add access points for the expanded gateway
      gatewayData.wtp_array.forEach((wtp, wtpIndex) => {
        const wtpId = `gateway-${gatewayIndex}-wtp-${wtpIndex}`;
        const wtpX = gatewayX + wtpIndex * ROOT_X_SPACING;
        nodes.push({
          id: wtpId,
          position: {
            x: gatewayX + wtpIndex * ROOT_X_SPACING,
          ...
          data: {
            label: (
              ...
              >
                ...
                <div className="my-2">
                  <div className="bg-green-50 p-2 rounded font-primary">
                    <span className="text-green-600 font-semibold">Users</span>
                    <p className="text-green-600 mt-1 font-semibold">
                      {wtp.radios_array?.length || 0}
                    </p>
                  </div>
                </div>
              </div>
        ...
        if (expandedAccessPoint === wtpId && wtp.radios_array) {
          const totalWidth = wtp.radios_array.length * RADIO_SPACING;
          // Calculate the starting X position to center the radio cards
          const startX = wtpX - totalWidth / 2 + RADIO_CARD_WIDTH / 2;
          wtp.radios_array.forEach((radio, radioIndex) => {
            const radioId = `${wtpId}-radio-${radioIndex}`;
            nodes.push({
              id: radioId,
              position: {
                x: startX + radioIndex * RADIO_SPACING,
                y: ROOT_Y + 2 * CHILD_Y_SPACING,
              },
              data: {
                label: (
